...
无论如何，在硬件层面，这归结为与纹理采样操作相关联的一袋状态——采样器状态、要使用的纹理/格式等——需要存放在某个地方（第二部分将解释在流水线架构中管理状态的各种方式）。因此，确实存在多种方法，从“每次任何状态更改时都刷新流水线”到“在采样器中完全无状态，并随每个纹理请求发送完整状态集”，以及介于两者之间的各种选项。这些你无需担心——这是硬件架构师拿来做成本效益分析、模拟几种工作负载然后选择最优方法的事情——但值得重复强调的是：作为 PC 程序员，不要假设硬件会遵循任何特定模型。
