...
1.  预计算出每个三角形的三个边的边方程系数 aaa、bbb、ccc；
    
2.  对于每条边，提前计算出 ia+jbia + jbia+jb，其中 0≤i,j≤70 \le i,j \le 70≤i,j≤7；
    
3.  只需算一次左上角像素的边方程值；
    
4.  然后通过**并行加法**计算整块 8×88×88×8 像素的边方程值；
...
E(x,y)=ax+by+cE(x, y) = a x + b y + cE(x,y)=ax+by+c

-   **线性函数的极值只会出现在 tile 边界的 4 个角**；
    
-   所以我们只需要计算 4 个角的 E 值；
    
-   根据 a,ba, ba,b 的符号可以判断最大值出现在哪个角；
