...
对于如纹理资源这样的稀疏状态集，可以分配一组物理描述符（比如 128 个），按需分配并重命名，不必为每个状态副本保留所有资源。
对于诸如采样器或纹理着色器资源视图这样的状态，理论上可能一次性要设置大量项，但实际应用中往往不会如此。你不会仅仅因为要支持两个“飞行中”状态集，就为 2×128 个活动纹理预留状态空间。对于此类场景，可以使用类似**寄存器重命名**的方案——维护一个 128 个物理纹理描述符的池子。只有在某个着色器真的需要 128 纹理时，状态变更才会变慢（那就认栽吧）；而在更常见的状况下，应用使用少于 20 个纹理，你就有充足空间同时保留多个状态版本。
