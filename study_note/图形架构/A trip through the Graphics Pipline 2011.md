...
从这个缓冲 buffer 开始，我们就将进入实际的命令处理前端。**这个指令处理前端基本上是一个状态机，知道如何解析指令（硬件特定格式）。** 有些命令处理的是 2D 渲染操作--除非有一个单独的命令处理器来处理 2D 的东西，而 3D 前端根本看不到它。不管是哪种方式，现代 GPU 上仍然隐藏着专门的 2D 硬件，就像在那个模具上的某个地方有一个 VGA 芯片，仍然支持文本模式、4 位/像素位面模式、平滑滚动和所有这些东西。好在没有显微镜的情况下，能在裸片上找到这些东西，总之，那些东西是存在的，但从此以后我就不再提了。还有一些命令，实际上是把一些图元交给 3D/着色器管道，我将在接下来的文章中介绍它们。还有一些命令，出于各种原因（以及在各种管道配置中），进入 3D/shader 管道，但从未渲染任何东西；这些将在以后的章节中讨论。
