...
1.  **分别生成**
    ...
    -   一条流水用来生成 iaiaia（只需将 aaa 与 iii 做乘法或查表）
        
    -   另一条流水用来生成 jbjbjb
        
2.  **Carry-Save 加法**
    
    -   CSA 能同时把 3 个操作数(ia, jb, c)(ia,\ jb,\ c)(ia, jb, c) 在一个时钟周期内“部分相加”，输出一对**未合并的**“部分和”（sum）和“部分进位”（carry）位向量。
        
    -   这种并行的“部分相加”不需要沿整个数据宽度做进位级联，因此时延极短。
        
3.  **最终合并**
    
    -   再用一个常规的二元加法器（ripple-carry 或更快的前缀加法器）把 CSA 的“sum”和“carry”相加，得到最终完整的 EijE_{ij}Eij​。
