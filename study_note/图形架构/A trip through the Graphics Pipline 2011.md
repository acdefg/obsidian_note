...
好了，到目前为止我们已经掌握了所有必要的算法和理论——接下来看看如何把这些“新玩具”与之前讨论的模块串联起来。

1. **三角形设置（Triangle Setup）**

   * 在此阶段要额外为深度 $Z$ 和其它插值属性做好准备。工作量略有增加，但无可避免。

2. **粗光栅化（Coarse Rasterization）**

   * 按 8×8 或其他 tile 大小快速筛出可能被覆盖的 tile。

3. **分层 Z（Hierarchical Z）**

   * 放在粗光栅化之后、细光栅化之前执行。
   * 每个 tile 存储一个**保守的最大深度**（Z-max），只要新三角形在此 tile 的最小深度 $Z_\min$ 大于存储的 Z-max，就可整块剔除。
   * Z-max 可存于片上 SRAM，8×8 tile 时，大约只需几十到一百多千位的存储即可覆盖 2048×2048 的分辨率。
   * 当驱动或上下文切换导致分层 Z 内存不足时，可能只能对部分 render targets 或区域启用，或者直接关闭分层 Z。

4. **分层 Stencil**

   * 原理类似分层 Z，通常与分层 Z 并行执行；具体细节厂商多有机密。

5. **细光栅化（Fine Rasterization）**

   * 在像素级别计算 coverage，输出每个像素或子像素的遮罩。

6. **提前 Z（Early Z）**

   * 紧跟细光栅化之后、像素着色之前执行深度/模板测试。
   * 如前所述，若像素着色器不输出新深度，就能安全使用 Early Z；否则，需回退到 Late Z。

7. **像素着色（Pixel Shading）**

   * 此后才调用像素着色器进行真正的颜色、纹理等计算。

8. **延迟 Z（Late Z）**

   * 像素着色器可能写深度或调用 discard，这时在着色后再做一次深度/模板测试和写入。
