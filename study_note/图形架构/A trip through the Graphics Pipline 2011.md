...
```mermaid
flowchart TD
  subgraph GDDR_Memory["GDDR Memory"]
    GDDR1[GDDR]
    GDDR2[GDDR]
    GDDR3[GDDR]
    GDDR4[GDDR]
  ...
  subgraph Controllers["Memory Controllers"]
    CTRL1[Controller]
    CTRL2[Controller]
  end
  ...
  CTRL1 --> MEMHUB[Memory Hub]
  CTRL2 --> MEMHUB

  MEMHUB --> MMU[MMU<br/>(or just a mux)]

  subgraph MMU_IO[""]
    MMU -->|Address| GPU[rest of GPU]
    MMU -->|Load data| GPU
    GPU -->|Store data| MMU
    GPU -->|Control| MMU
  end

  MMU --> PCIe[PCIe Host Controller76 lanes to host]
