...
ZZZ（深度）和所有由顶点着色器输出的属性都会跨三角形进行插值。下面给出要点总结：

1.  **为何不能直接线性插值属性？**  
    如果你直接在屏幕空间对颜色、纹理坐标 (s,t)(s,t)(s,t) 等属性做线性插值，结果并不正确（除非你选择了“无透视”插值模式）。
    
2.  **透视正确插值的做法**
    
    -   首先在线性空间中插值：
        
        1w,sw,tw\frac{1}{w},\quad \frac{s}{w},\quad \frac{t}{w}w1​,ws​,wt​
        
        其中 www 是齐次裁剪空间中的 www 分量。
        
    -   然后对每个像素点取倒数：
        
        w=(1w)−1w = \bigl(\tfrac{1}{w}\bigr)^{-1}w=(w1​)−1
    -   最后恢复原始属性：
        
        s=sw ×w,t=tw ×ws = \frac{s}{w}\,\times w,\quad t = \frac{t}{w}\,\times ws=ws​×w,t=wt​×w
3.  **使用重心坐标的优化**  
    如果需要插值多个属性，更高效的做法是：
    
    -   在原始裁剪空间三角形中，用透视插值计算出像素的两条重心坐标 λ0,λ1\lambda_0,\lambda_1λ0​,λ1​；
        
    -   之后对任意顶点属性都只需做普通的线性组合，无需再乘以 www。
...
-   计算 λ0w\tfrac{\lambda_0}{w}wλ0​​ 和 λ1w\tfrac{\lambda_1}{w}wλ1​​ 需要：
...
因此在屏幕空间可以对 ZZZ 直接**线性插值**。最终我们得到一个平面方程：

Z(X,Y)=a X+b Y+cZ(X,Y) = a\,X + b\,Y + cZ(X,Y)=aX+bY+c

只需带入像素坐标 (X,Y)(X,Y)(X,Y)，即可计算出该点的深度值。

> **要点**：对某一点插值 ZZZ 仅需两次乘法与一次加法，这也是 GPU 廉价乘加单元（MAC）无处不在的原因之一。

---

### 总结

-   属性透视正确插值需先插值 attrw\tfrac{attr}{w}wattr​ 再乘以 www；
    
-   更优方案是用重心坐标先算 λi\lambda_iλi​，再做线性组合；
    
-   对 Z=zwZ = \tfrac{z}{w}Z=wz​ 可直接线性插值，得到形式为 aX+bY+caX + bY + caX+bY+c 的平面方程；
    
-   每像素深度插值仅需两次乘法一次加法，充分利用了 GPU 的乘加硬件。