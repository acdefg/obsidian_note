...
从这个缓冲 buffer 开始，我们就将进入实际的命令处理前端。**这个指令处理前端基本上是一个状态机，知道如何解析指令（硬件特定格式的指令）。** 有些指令处理的是 2D 渲染操作--除非有一个单独的指令处理器来处理 2D 的东西，而 3D 前端根本看不到它。不管是哪种方式，现代 GPU 上仍然隐藏着专门的 2D 硬件，就像在片上的某个地方有一个 VGA 芯片，仍然支持文本模式、4-bit/pixel 位面模式、平滑滚动和所有这些东西。好在没有显微镜的情况下，能在裸片上找到这些东西，总之，那些东西是存在的，但从此以后我就不再提了。还有一些命令，实际上是把一些图元交给 3D/着色器管道，我将在接下来的文章中介绍它们。还有一些命令，出于各种原因（以及在各种管道配置中），进入 3D/shader 管道，但从未渲染任何东西；这些将在以后的章节中讨论。


命令会从缓冲区进入命令处理器的前端，这是一个解析硬件格式命令的 **状态机（state machine）**。命令类型包括：
...
改变状态（比如纹理绑定）在程序看来只是改变量，但对并行的 GPU 而言必须慎重处理。几种常见策略包括：

#### 1. 管线刷新（Flush）

最简单的方式是更改状态时强制前面的任务完成，即执行 **部分管线刷新**。这种方法简单，适合较少批次、较少三角形时，但在现代复杂场景中成本较高。

#### 2. 无状态设计

某些模块可以设计为 **完全无状态**，每个周期都将当前状态附加在数据上，向下游传递。但若状态数据太大（如纹理采样配置），此方法就不现实了。

#### 3. 双缓冲（State Slot 双槽）

用两个槽位存储状态，当前任务用 Slot 0 时，可以修改 Slot 1。切换状态只需一个比特位来选择使用哪个槽。适用于状态变更频繁但内容不大的场景。
