---
title: A trip through the Graphics Pipline 2011
tags: ["note"]
created: 星期日, 六月 28日 2026, 2:58:15 下午
modified: 星期日, 六月 28日 2026, 6:07:01 晚上
---
...
你第一次使用某个资源或着色器时可能会卡顿 —— 因为驱动延迟了资源的创建与编译。为了确保资源真正被创建，图形程序员经常会发一个假的绘图调用 " 热身 " 资源。虽然丑陋但从 1999 年以来就是这样，习惯就好。
...
- API/驱动概念中，系统内存指的就是这块内存区域，与 GPU 的**专用显存（Video Memory / VRAM）**相对。
	...
	- **显存**（Video Memory / VRAM）
		- 专门给 GPU 用的高速内存，通常 GDDR 或 HBM，带宽非常高（上百 GB/s），但往往延迟也更高。
...
> **位平面模式**
> 是一种图形显示模式。在计算机图形学中，位平面是用于表示图像像素的二进制位的集合。每个位平面包含图像的一个二进制位，多个位平面组合在一起可以表示不同颜色深度的图像。例如，一个 8 位的图像就有 8 个位平面，每个位平面表示图像的一个二进制位。这种模式主要用于描述图形硬件如何处理和显示图像数据。
...
- **HS — Hull 着色器（Hull Shader）**  
	...
	- 这是一个可编程的阶段，开发者可以指挥 GPU 如何对顶点进行细分操作，但还不会真正进行细分，就像是指挥流水线上的工人说：" 来，帮我把这给这根钢筋中间打上三个标记，好让后面的工人在上面安上旋钮。"
- **TS — 细分器阶段（Tessellator Stage）**  
	根据 HS 提供的细分因子，生成新的顶点位置和图元连通性，实现曲面细分。
	- 真正的细分阶段；尽管开发者无法在这个阶段进行编程，但 GPU 将会根据 Hull-Shader Stage 中的标记进行细分；就像是流水线上的工人木讷地照着传过来的钢筋上的标记安装上旋钮
...
- **GS — 几何着色器（Geometry Shader）**  
	...
	- 在这个阶段，开发者可以控制 GPU 对顶点进行增删改操作。几何着色器与顶点着色器都可以对顶点的坐标进行修改，但几何体着色器并行调用硬件困难，并行程度低，效率和顶点着色器有很大的差距；如果不是要做顶点增、删这些仅仅能用几何着色器实现的效果，那么还是用顶点着色器来完成吧。
- **SO — 流式输出（Stream-Out）**  
...
## 输入组装阶段 IA（Input Assembler Stage）
...
所有这些都是几乎所有实现的共同点。也有一些差异；AMD 的硬件过去直接遵循 HLSL/GLSL 和着色器字节码所暗示的 4 宽度 SIMD（尽管他们似乎最近正在逐渐远离这种设计），而 NVIDIA 则在一段时间前决定将 4 路 SIMD 转化为标量指令。不过，所有这些内容都已经可以在网上找到！
...
在纹理采样过程中，图形处理器（GPU）会根据片段（fragment）到纹理的采样距离来选择合适的 MIP 级别进行采样。这个过程称为 MIP 映射选择或 MIP 级别选择。
...
**也就是说，任何纹理缓存都是巨大的收益（因为它能将双线性采样从大约 4 次内存访问降到 1.25 次）**，但与 CPU 或着色器核心的共享内存不同，**将 L1 缓存从 4 KB 扩大到 16 KB 收益甚微；** 无论如何，我们都在通过 cache 传输体量更大的纹理数据。
...
E(x, y) = a x + b y + c
$$
对于某个像素 (x,y)(x, y)(x,y)，只要我们知道该像素处的 E(x,y)E(x, y)E(x,y)，就可以非常快速地得到它相邻像素处的边方程值：
...
- 分别计算 $ia$ 和 $jb$
- 使用一个 **carry-save adder**（也叫 3:2 reducer）将：
$$
 ia+jb+c
$$
...
- 大三角形通常会横跨多个 tile，因此会被发送给**所有光栅化器**；
- 小三角形可能只命中一个瓦片区域，因此只发送给**拥有该 tile 的光栅化器**。
...
- 如果屏幕上有几百个非常小的三角形，全都落在同一个 tile 中，那么：
	- **只有一个光栅化器会收到这些工作**；
	- **其余光栅化器则会处于空闲状态**；
	- 导致**严重的负载不平衡**，而这基本无解。
...
好吧，所以我们接收到的输入**不再只是单一的一条线性流**（即 quad 的坐标加上覆盖掩码），而是**两到四条并行的数据流**。但我们仍然需要把这些工作**分发给数百个着色器单元**。是时候再引入一个**分发单元（dispatch unit）** 了！这当然意味着还需要一个新的**缓冲区（buffer）**。

那我们每次要送到着色器的批次有多大呢？这里我再次引用 NVidia 的数据 [nvidia.com/content/PDF/fermi\_white\_papers/NVIDIA\_Fermi\_Compute\_Architecture\_Whitepaper.pdf](http://www.nvidia.com/content/PDF/fermi_white_papers/NVIDIA_Fermi_Compute_Architecture_Whitepaper.pdf)，仅仅是因为他们在公开白皮书中提到了这个数字；AMD 也很可能有相应的说明，但我对他们的术语不熟悉，因此没法直接查到相关内容。

无论如何，NVidia 分发给着色器单元的单位是 **32 个线程**，他们称之为一个 **"Warp"**。每个 quad 有 4 个像素（每个像素可以看作一个线程），因此我们每发出一个 shading 批次，**就需要从 rasterizer 收集 8 个 quad** 才能凑够一个 Warp（当然如果发生 shader 切换或流水线刷新，可能会发出较小的批次）。

---

现在也是解释**为什么我们处理的是 2×2 的 quad，而不是单个像素**的好切入点。

最主要的原因是**导数（derivatives）**。

纹理采样器需要根据屏幕空间中的纹理坐标导数，来选择 mipmap 层级以及进行纹理过滤（我们在第 4 部分已经看到过）；而从 Shader Model 3.0 开始，**像素着色器中也可以直接使用导数指令**，访问这些导数。

在一个 quad（2×2 像素）中，每个像素都有水平和垂直方向的邻居，这可以用来通过 [**有限差分（finite differencing）**](http://en.wikipedia.org/wiki/Finite_difference) 估算 x 和 y 方向上的参数导数（本质上就是做几次减法）。这提供了一种**非常廉价地获取导数**的方法，代价是必须始终以 2×2 像素为一组进行 shading。

在大三角形的内部，这并不会带来什么问题。但在小三角形边缘，这就意味着 **25% 到 75% 的 shading 计算可能是浪费的**。

为什么会浪费？因为即使 quad 中某些像素被掩码屏蔽（即不可见），它们**依然要参与 shading 运算**，这是为了确保 quad 中**可见像素的导数计算是正确的**。这些不可见但仍然被着色的像素，被称为 **"helper pixels"（辅助像素）**。

---

下面是一个关于小三角形的示意图：
![|300x300](https://fgiesen.wordpress.com/wp-content/uploads/2011/07/quad_coverage.png)
小三角形覆盖了 4 个 quad，但**只有 3 个 quad 中生成了可见像素**。而在这 3 个 quad 中，每个 quad 实际上**只有一个像素被覆盖**（每个像素区域的采样点用黑色圆圈表示）——被填充的像素用红色表示。

每个被部分覆盖的 quad 中，其余未被覆盖的像素则是 **helper pixels**，在图中用较浅的颜色表示。

这个示意图应该清楚地表明，对于小三角形来说，被 shading 的总像素中有很大一部分都是 helper pixels。这一问题已经引起了一些研究关注，比如尝试合并相邻三角形的 quad 以减少浪费。

然而，尽管这些优化手段很巧妙，**现行图形 API 的规则并不允许这样做**，而且**目前的硬件也没有实现这类优化**。当然，如果哪一天硬件厂商认为这些因为 quad 粒度过粗而造成的 shading 浪费问题严重到必须解决，那这种情况很可能会发生改变。

## 属性插值

像素着色器的一个独特特性是**属性插值**。  
所有其他类型的着色器 —— 不论是我们已经讲过的（如顶点着色器 VS），还是后续要讲的（几何着色器 GS、细分控制着色器 HS、细分评估着色器 DS、计算着色器 CS）—— 它们的输入都**直接来自前一个着色器阶段或显存**。

但像素着色器不一样，它们在前面**多了一步插值操作**。  
我在前一部分讨论深度值 Z 的时候已经提到过插值，它就是我们接触到的第一个被插值的属性。

其他插值属性的处理方式基本也一样：  
在**三角形设置阶段**（Triangle Setup）时，GPU 会为每个要插值的属性计算一个平面方程（plane equation）。  
有些 GPU 可能会延迟这一步（比如在确定三角形至少有一个 tile 通过了层次化 Z 测试之后才进行），但这些细节我们暂时不考虑。

之后在**像素着色阶段**，会有一个独立的单元，根据 quad 的像素位置和刚才计算的平面方程，对属性进行插值。

---

### 💡 更新说明

Marco Salvi 在文末评论中指出：

> 尽管早期 GPU 中确实有专用的 " 插值器 "（interpolator），但**现在的趋势是：直接提供重心坐标（barycentric coordinates）给 shader，用于插值**。

换句话说：

- 硬件返回的是每个像素点的重心坐标 λ₀, λ₁, λ₂；
- 然后在着色器中自己执行插值；
- 对每个属性，只需做 **两次乘法一次加法（2×MUL + 1×ADD）**；
- 这样插值操作可以直接在 shader ALU（算术逻辑单元）中完成。
...
#### ✅ 1. **常量插值（constant interpolation）**

- 所有像素都采用一个固定的值；
- 通常是三角形中某个 " 主顶点 "（leading vertex）的属性值；
- 哪个顶点是 " 主顶点 " 在图元组装阶段决定；
- 硬件可能通过快捷通道来处理这种插值，也可以照常设置一个常数的平面方程，两种方法都可以。

---

#### ✅ 2. **无透视插值（no-perspective interpolation）**

- 不进行透视校正；
- 插值时不会将每个顶点的属性除以其对应的 `w` 值；
- 这意味着可以直接对属性值在 X、Y 屏幕空间中建立平面方程；
- **比透视插值便宜得多**，因为不涉及除法和非线性校正；
- 常用于对精度要求不高的参数，如屏幕空间坐标、mask、简单的颜色等。

---

### 🎯 对比一下

|插值方式|是否透视校正|平面方程建立方式|计算代价|
|---|---|---|---|
|透视插值|✅ 是|属性值 / w，然后建方程|高（准确）|
|无透视插值|❌ 否|直接对属性值建方程|低（近似）|
|常量插值|-|不插值，取主顶点值|最低|
## 🎯 Centroid 插值是个棘手的问题

接下来我们来看 "**centroid（质心）插值**"。

这其实是一个**标志（flag）**，而不是一种单独的插值模式；它可以与**透视插值（perspective）**或**无透视插值（no-perspective）**配合使用（但**不能用于常量插值（constant）**，因为那就没有意义了）。

"Centroid" 这个名字取得不太好，而且如果没有启用多重采样（MSAA），它根本不会起作用。  
但在启用了多重采样的情况下，它是一个用于解决真实问题的、**有点 "hack" 的方案**。

---

### 🧩 问题的背景

在多重采样开启时，GPU 会在**多个采样点**上评估图元对像素的覆盖情况（coverage），但 **着色操作仍然每像素只做一次**。  
这时，像纹理坐标这样的属性会在**像素中心位置**进行插值，就好像整个像素都被图元覆盖一样。
...
- 图中小圆圈表示 **4x MSAA 的四个采样点**；
- 中间的大圆圈是 **像素中心位置**。


注意，这个大圆圈 **在三角形之外**，而我们会在它处插值属性值。  
这其实就是在做**线性外推（extrapolation）**，并不是插值！

如果程序使用纹理图集（texture atlas），就可能会造成非常错误的采样 —— 比如你想采一块布料纹理，结果跑去采了铁皮贴图。

根据三角形的大小不同，像素中心位置的值可能会偏得很远。

---

## ✅ Centroid 插值的解决方案

Centroid 插值就是为了解决这个问题。

早期的解释是：

> GPU 会取所有**被图元覆盖的采样点**，计算它们的 " 质心 "，然后在这个位置进行属性采样 —— hence the name "centroid"。

接着大家又会补一句：

> 这是个概念模型，GPU 实际实现可以不同 —— 只要保证采样点在三角形内即可。
...
- **如果所有采样点都被三角形覆盖**：  
	那么插值点就是像素中心，这时正好也是所有采样点的 " 质心 "，所以没区别。

- **如果只有部分采样点被覆盖**：  
	GPU 就会**从那些被覆盖的点中任选一个**进行插值。  
	被覆盖的采样点必然都在图元内，所以结果仍然有效。

> 一开始这个选择是完全由硬件自行决定的；但现在（比如在 DX11 中）已经有了明确规定，用来确保不同硬件结果一致。不过对于大多数 API 使用者来说这无关紧要。  
> 说到底，这仍然是一个 " 工业级胶带 " 式的解决方法 —— 有点丑，但能用。

---

### 😅 但它有副作用

- 它**会破坏导数（derivative）计算**，尤其是对那些被部分覆盖的像素 quad。
- 因为计算导数通常依赖 quad 中 2×2 像素的结构对比，而其中一些像素的插值点现在是 " 偏离中心 " 的，就会干扰导数结果。
- 但没办法，只能接受 —— " 这可能是结实的工业胶带，但终究只是胶带。"

---

### 🧪 DX11 新增特性：Pull-Model 插值
...
- 正常情况下，属性插值在**像素着色器开始执行前就已经完成**；
- 而使用 pull-model 插值时，插值的操作是通过 **像素着色器中的指令主动触发的**。


这带来一些新能力：

- 着色器可以**自行计算希望在哪个位置插值**；
- 或者只在某些分支中插值某些属性，在其他分支中不插值（节省开销）；
- 本质上，像素着色器在运行时可以**发出新的插值请求**给插值单元。
...
|插值方式|描述|
|---|---|
|**Centroid 插值**|在 MSAA 中避免像素中心不在图元内造成的错误采样，采样点选自被覆盖的 MSAA 子像素。|
|**Pull-Model 插值**|像素着色器在运行时发起插值请求，而不是在开始前自动完成，带来更灵活的插值控制能力。|

---

## 实际的着色器执行体（The actual shader body）

再强调一次，关于着色器的基本原理，API 文档中已经讲得很清楚了，所以我不会详细讲每一条指令是如何工作的——一般来说，它们的表现方式就是你预期的那样。但关于像素着色器的执行过程，有一些值得一讲的细节。

---

### 📦 首先是：纹理采样

等等，我不是在第 4 部分里已经大谈特谈过纹理采样器了吗？  
没错，但那是从**纹理采样器本身**的角度讲的。

还记得我提到过：**纹理缓存未命中是很常见的**，所以采样器的设计通常需要**能够承受每个请求（16~32 个像素）至少一次访问主存**，而不会导致流水线停滞。  
那是好几百个周期的延迟！  
如果在此期间让 ALU（算术逻辑单元）空闲，那就太浪费了。
...
- **发出纹理采样请求后就切换到另一个任务批次（batch）继续执行**；
- 如果那个批次也发出纹理采样请求或者执行完毕，就会切换回先前的批次，看看采样结果是否已返回；
- 只要每个着色器单元有多个批次可用，它就可以一直忙碌着。


这充分利用了 ALU，但代价是：

- 每个批次的 " 完成延迟 " 被拉长了；
- 又是一个典型的 " 延迟 vs 吞吐率 " 权衡。


不过，到现在你应该已经明白了 —— **GPU 永远优先考虑吞吐率！**

---

### 🧠 Warp 和寄存器压力

要想让一个着色器单元同时运行多个批次（NVIDIA 的术语是 **Warp**，AMD 称之为 **Wavefront**），需要更多的**寄存器资源**。

- 如果一个着色器程序使用了大量寄存器，那么一个单元能够保存的 Warp 数量就会减少；
- Warp 数越少，遇到全部 Warp 都在等纹理采样结果的情况就更容易发生；
- 一旦所有批次都处于 " 等待中 "，那你就只能停住，直到某个结果回来。


这确实不幸，但硬件资源是有限的 —— 内存不够就是不够，没什么好说的。

---

### 🔀 分支与循环的困扰

还有一个我们没讲的点是：**动态分支（if/else）和循环（loop）**。

- 在 GPU 着色器单元中，**每个 Warp 里的所有线程是锁步执行的（SIMD）**；
- 这意味着：

> 如果 Warp 中有任意一个线程需要进入 `if` 分支，**所有线程都必须执行这个分支**，  
> 哪怕它们中的大多数根本不想执行这一分支 —— 它们会通过 " 条件执行（predication）" 忽略计算结果，但执行还是要做的。

- 同样的，`else` 分支也必须被全体线程执行。


这个机制在 " 条件一致性较高 " 的场景下效果不错，但如果分支是随机的，那就很糟糕了：

> **最坏情况：你永远都在执行 if 和 else 的两条路径。**

循环也是类似的：只要 Warp 中有一个线程还要继续循环，**全体线程都得陪着它跑下去**。

---

### ❌ Discard 指令的副作用

像素着色器还有一个特有的指令叫做 `discard`（丢弃当前像素），即像素不写入帧缓冲。

- 如果整个 batch 中所有像素都被 discard，着色器单元就可以停止并调度另一个 batch；
- 但只要有一个像素还没 discard，**其余线程也必须跟着执行下去**。


DX11 增加了更细粒度的控制：

> 你可以直接在着色器里写出像素的 " 覆盖掩码（coverage mask）"，  
> 这将会与原始图元覆盖 + Z-test 覆盖做 AND 操作，**防止着色器写入三角形范围之外的像素**（以保证一致性）。

这可以实现每个样本（而非整个像素）的 discard，  
比如你想用着色器里的自定义抖动算法实现 Alpha-to-Coverage 的效果。

---

### 📏 写入深度值：杀死 Early-Z

像素着色器也可以写出像素的深度值（Z 值），  
这个功能早就支持了。

但我想说：  
**这是摧毁 Early-Z、Hierarchical-Z、Z 压缩等所有优化路径的绝佳方式。**

现在你已经了解了这些机制背后的原理，自然明白为什么这么说了。🙂
...
- 通常是每个 render target 一组 4 分量向量；
- 当前最多可以输出 **8 个 render target**；
- 着色器完成后，这些输出会被送到 D3D 管线中的 **Output Merger（输出合并器）**。


Output Merger 会在下一节中讲。

---

### 🆕 DX11 新功能：写入 UAV（无序访问视图）

从 D3D11 开始，像素着色器可以写入 **UAV（Unordered Access Views）** ——  
这个功能之前只有 **计算着色器** 和 **像素着色器** 可以使用。

- UAV 通常是 compute shader 中的 " 输出目标 "；
- 不同于传统 render target，UAV 可以**随意写入任意位置**，且没有 API 顺序保证（即 " 无序 "）；
- 所以名字叫 Unordered Access View。


这里只是提一下，后面讲计算着色器时会更详细地展开。

---

## 📚 附录：补充说明

在评论区里，Steve 指正了 AMD 的术语（最初我忘了叫 "Wavefront"），  
并贴了 Kayvon Fatahalian 的一份精彩演讲材料，详细讲解了 GPU 中着色器的执行方式，  
图示比我愿意画的多得多 😂，**强烈推荐感兴趣的读者去看看。**

---

### ✅ 总结

- GPU 中的像素着色器以 **吞吐优先** 为目标设计；
- 批次并行、Warp 切换、SIMD 锁步执行、predication、discard、覆盖掩码、深度写入等机制相互配合；
- 动态分支和循环的性能影响显著，MSAA 和纹理采样也是影响关键；
- DX11 进一步增强了灵活性，如输出 coverage mask 和写入 UAV。

## new
### 第一部分：像素处理的 " 分叉 " 与 " 合并 "

在这一部分，我将处理像素处理的前半部分：**调度（dispatch）**和**实际的像素着色（pixel shading）**。

实际上，这正是大多数图形程序员在谈论像素处理时脑海中主要想到的内容；至于我们将在下一部分遇到的 **Alpha 混合（alpha blend）**和**后期 Z 测试（late Z）**阶段，看起来不过像是事后的一点补充。正如我们将看到的，在硬件层面故事要稍微复杂一些——这也是我将像素处理分成两部分的原因。

但我有些操之过急了。当我们进入这个阶段时，要着色的像素（或者确切地说是 **2x2 的四像素块，Quads**）的坐标以及相关的**覆盖掩码（coverage masks）**，正从光栅化器/Early-Z 单元传送过来——正如我上次指出的，此时三角形的顺序与应用程序提交的顺序完全一致。

我们要做的就是获取这一线性的、连续的工作流，将其分发给数百个着色器单元，一旦结果返回，我们需要将其合并回内存更新的一个线性流中。

> 这是一个教科书式的**" 分叉/合并 "（fork/join）并行模式**示例。这一部分主要处理 " 分叉 " 阶段，即我们扩展宽度的阶段；下一部分将解释 " 合并 " 阶段，即我们将数百个流合并回一个流的过程。

但在那之前，关于光栅化我还有几句话要说，因为我刚才告诉你的 " 只有一个四像素块流传入 " 的说法并不完全正确。

---

### 光栅化的真相：多路并行（Going Wide）

为了给自己辩护一下，我之前告诉你们的情况在很长一段时间里确实是真的，但光栅化是管线中的串行部分，当你把**超过 300 个着色器单元**投入到一个问题中时，管线的串行部分往往会成为瓶颈。

因此 GPU 架构师开始使用**多个光栅化器**；截至 2010 年，NVidia 使用四个光栅化器，而 AMD 使用两个。顺便提一下，NV 的演示文档中也有一些关于保持 API 顺序要求的说明。特别是，正如我上次提到的，你确实需要在光栅化/Early-Z 之前将图元（primitives）重新排序；如果想在 Alpha 混合之前才做排序（你可能倾向于这么做），是行不通的。

**光栅化器的工作分配机制：**

光栅化器之间的工作分配是基于我们在 Early-Z 和粗粒度光栅化中已经见过的**图块（tiles）**进行的。

1. 帧缓冲区被划分为图块大小的区域，每个区域分配给其中一个光栅化器。
2. 在图元装配（setup）之后，会查询三角形的包围盒以确定将哪些三角形交给哪个光栅化器。
3. 大三角形总是会被发送给所有的光栅化器，但小三角形可能只覆盖一个图块，因此只会被发送给拥有该图块的那个光栅化器。

这种方案的美妙之处在于它只需要更改工作分配和粗粒度光栅化器（负责遍历图块）；所有只处理单个图块或四像素块的部分（即从层级 Z 往下的管线）都不需要修改。

**潜在问题：**

问题在于现在你是根据屏幕位置来划分工作的；这可能会导致光栅化器之间严重的**负载不平衡**（想象一下几百个微小的三角形都挤在一个图块内的情况），而你对此真的无能为力。但好的一面是，所有给管线增加排序约束的东西（Z 测试/写入顺序、混合顺序）都是依附于特定的帧缓冲区位置的，所以屏幕空间细分是在不破坏 API 顺序的前提下进行的——如果情况并非如此，**基于图块的渲染器（tiled renderers）**就无法工作了。

---

### 调度与批次：为什么我们需要 " 更宽 " 的通道

好吧，所以我们得到的不仅仅是一个线性的四像素块坐标加覆盖掩码流，而是两到四个。我们仍然需要将它们分发给数百个着色器单元。是时候再来一个**调度单元**了！这首先意味着需要另一个缓冲区。

但是我们要发送给着色器的**批次（batches）**有多大呢？

这里我再次使用 NVidia 的数据，仅仅因为他们在公开白皮书中提到了这个数字；AMD 可能也在某处声明了该信息，但我不熟悉他们的术语，所以没法直接搜索到。

- 不管怎样，对于 NVidia，分发给着色器单元的单位是 32 个线程，他们称之为**" 线程束 "（Warp）**。
- 每个**四像素块（Quad）**有 4 个像素（每个像素可以作为一个线程处理）。
- 所以对于我们要发布的每个着色批次，我们需要从光栅化器抓取 **8 个传入的四像素块**，然后才能发送一批次给着色器单元（如果有着色器切换或管线刷新，我们可能会发送少于这个数量）。

#### 为什么要处理 2x2 的四像素块（Quads）？

这也是解释为什么我们要处理 2x2 像素的四像素块而不是单个像素的好时机。主要原因是**导数（derivatives）**。

纹理采样器依赖纹理坐标的屏幕空间导数来进行 mip-map 选择和过滤（正如我们在第 4 部分中看到的）；而且，从 Shader Model 3.0 及以后，同样的机制以导数指令的形式直接提供给像素着色器使用。

在一个四像素块中，每个像素在同一个块内都有水平和垂直的邻居；这可以用来通过**有限差分（finite differencing）**（归结起来就是几次减法）来估算 x 和 y 方向上参数的导数。这为你提供了一种非常廉价的获取导数的方法，代价是总是必须一次性着色 2x2 的像素组。

#### 辅助像素（Helper Pixels）的代价

这在大三角形内部不是问题，但意味着对于三角形边缘生成的四像素块，大约 **25-75% 的着色工作是浪费的**。

这是因为四像素块中的所有像素，即使是被掩盖（masked）的像素，也会被着色。这对于为四像素块中可见的像素产生正确的导数是必要的。那些不可见但仍被着色的像素被称为**" 辅助像素 "（helper pixels）**。

这里有一个小三角形的插图：****
![[image-16.png|244x243]]

- 三角形与 4 个四像素块相交，但只在其中的 3 个块中生成可见像素。
- 此外，在这 3 个四像素块的每一个中，实际上只有一个像素被覆盖（每个像素区域的采样点描绘为黑色圆圈）——被填充的像素描绘为红色。
- 每个部分覆盖的四像素块中的其余像素是辅助像素，用较浅的颜色绘制。

这个插图应该清楚地表明，对于小三角形，被着色的像素总数中有很大一部分是辅助像素，这吸引了一些关于如何合并相邻三角形的四像素块的研究关注。然而，虽然这些优化很聪明，但目前的 API 规则不允许这样做，目前的硬件也不做这些。当然，如果硬件供应商在某个时候认为四像素块上的浪费着色工作是一个严重到必须解决的问题，这可能会改变。

---

### 像素着色器的独特功能：属性插值

像素着色器的另一个独特功能是**属性插值（attribute interpolation）**——所有其他着色器类型，无论是我们目前见过的（VS）还是我们要讨论的（GS, HS, DS, CS），都直接从前一个着色器阶段或内存中获取输入，但像素着色器在它们前面有一个额外的插值步骤。我在讨论 Z 时已经稍微谈到了这一点，那是我们看到的第一个插值属性。

其他插值属性的工作方式大致相同；在三角形装配期间为它们计算一个平面方程（GPU 可能会选择稍微推迟这个计算，例如直到已知三角形至少有一个图块通过了层级 Z 测试，但这不影响我们这里的讨论），然后在像素着色期间，有一个单独的单元使用四像素块的像素位置和我们刚刚计算的平面方程执行属性插值。

> **Update:** Marco Salvi 在评论中指出，虽然过去有专用的插值器，但现在的趋势是只让它们返回**重心坐标（barycentric coordinates）**以代入平面方程。实际的求值（每个属性两个乘加运算）可以在着色器单元中完成。

所有这些都不应该令人惊讶，但还有一些额外的插值类型需要讨论：

1. **常量插值（Constant）：** 正如其名，它们在整个图元上是恒定的，并从 " 主导顶点 "（leading vertex，在图元装配期间确定是哪个顶点）获取每个顶点属性的值。硬件可能有针对此的快速路径，或者只是设置一个相应的平面方程；无论哪种方式都可以。
2. **非透视插值（No-perspective）：** 这通常会以不同的方式设置平面方程；透视校正插值的平面方程要么是通过将每个顶点的属性值除以相应的 w 来进行基于 X、Y 的插值，要么是通过构建三角形边缘向量来进行重心插值。然而，当非透视插值属性的平面方程被设置为基于 X、Y 的插值且**不将每个顶点的值除以相应的 w** 时，计算成本最低。

---

### 棘手的 " 质心 " 插值（Centroid Interpolation）

接下来，我们有**" 质心 "（centroid）插值**。

这是一个标志，不是一个单独的模式；它可以与透视和非透视模式结合使用（但不能与常量插值结合，因为那没有意义）。它的命名也很糟糕，而且除非启用了**多重采样（multisampling）**，否则它就是一个空操作。

在开启多重采样的情况下，它是解决一个实际问题的稍微有点 " 黑客 " 性质的方案。

- **问题：** 使用多重采样时，我们在光栅化器中评估多个采样点的三角形覆盖范围，但我们要对每个像素只进行一次实际着色。诸如纹理坐标之类的属性将在**像素中心**位置进行插值，就好像整个像素都被图元覆盖一样。
- **后果：** 比如我们有一个被图元部分覆盖的像素；四个小圆圈描绘了 4 个采样点（这是默认的 4x MSAA 模式），而中间的大圆圈描绘了像素中心。注意大圆圈在图元之外，任何对它的 " 插值 " 值实际上都是**线性外推（linear extrapolation）**；如果应用程序使用**纹理图集（texture atlases）**，这是一个问题。根据三角形的大小，像素中心的值可能会偏离很远。


**解决方案：**

质心采样解决了这个问题。最初的解释是 GPU 取图元覆盖的所有样本，计算它们的质心，并在该位置采样（因此得名）。通常后面会补充说这只是一个概念模型，GPU 可以自由地以不同方式进行，只要它们选择的采样点在图元内即可。

如果你觉得硬件真的会去数覆盖的采样点，把它们加起来，然后除以总数，那欢迎加入我们的阵营（意指大家一开始都容易这么想，但事实并非如此）。实际发生的情况是：

1. 如果所有采样点都覆盖图元，插值照常进行，即在像素中心（对于所有合理的采样模式，它恰好是所有采样位置的质心）。
2. 如果并非所有采样点都覆盖三角形，硬件会选择其中一个覆盖图元的采样点并在那里进行评估。所有覆盖的采样点（根据定义）都在图元内，所以这行得通。


这种选择过去是任意的（即留给硬件决定）；我相信现在 DX11 实际上规定了它是如何完成的，但这更多是为了在不同硬件之间获得一致的结果，而不是 API 用户实际上会关心的东西。

如前所述，这有点 " 黑客 " 手段。它也倾向于搞乱具有部分覆盖像素的四像素块的导数计算——算你倒霉。我能说什么呢，这可能是**工业强度的胶带**，但它仍然是胶带。

**DX11 新特性：拉取模型（Pull-model）插值**

最后（DX11 新增功能！）还有 " 拉取模型 " 属性插值。常规属性插值在像素着色器启动之前自动完成；拉取模型插值将实际执行插值的指令添加到像素着色器中。这允许着色器计算自己的位置来采样值，或者只在某些分支中插值属性而在其他分支中不插值。归根结底，这是让像素着色器能够在运行时向插值单元发送额外请求。

---

### 着色器执行细节：吞吐量为王

关于实际的着色器主体，一般的着色器原理在 API 文档中已经解释得很清楚了，所以我不会谈论单个指令是如何工作的；通常，答案是 " 如你所期望的那样 "。然而，有一些关于像素着色器执行的有趣之处值得探讨。

#### 1. 纹理采样与延迟隐藏

第一个是：纹理采样！等等，我不是在第 4 部分已经长篇大论过纹理采样器了吗？是的，但那是纹理采样器方面的事情——如果你还记得的话，有一点是关于纹理缓存未命中非常频繁，以至于采样器通常被设计为每个请求（也就是 16-32 个像素，记得吗！）能承受至少一次主内存未命中而不发生停顿。

那是很多个时钟周期——数以百计。让完美的 ALU（算术逻辑单元）在这一切发生时闲置将是巨大的浪费。

所以着色器单元实际做的是在发出纹理采样后**切换到另一个批次**；然后当那个批次发出纹理采样（或完成）时，它切换回之前的批次之一，并检查纹理样本是否已经到达。只要每个着色器单元在任何给定时间都有几个批次可以处理，这就能很好地利用可用资源。

- **权衡：** 这确实增加了单个批次完成的延迟——再次强调，这是延迟与吞吐量的权衡。
- **结论：** 到现在你应该知道 GPU 上哪一方获胜了：**吞吐量（Throughput）！永远是吞吐量。**


这里要注意的一点是，同时保持多个批次（或 NVidia 硬件上的 "Warps"，或 AMD 的 "Wavefronts"）运行需要更多的寄存器。如果一个着色器需要大量寄存器，着色器单元能保留的 Warp 就会变少；如果它们变少了，那么在某个时刻你会耗尽那些不需要等待纹理结果的可运行批次，这种几率就会变高。如果没有可运行的批次，你就倒霉了，必须停顿直到其中一个批次取回结果。这很不幸，但这类事情的硬件资源是有限的——如果内存用完了，那就是用完了，没得商量。

#### 2. 动态分支与步调一致（Lockstep）

我还没谈到的另一点：着色器中的**动态分支**（即循环和条件语句）。

在着色器单元中，每个批次的所有元素的工作通常是**步调一致（lockstep）**进行的。所有 " 线程 " 在同一时间运行相同的代码。

这意味着 `if` 语句有点棘手：如果**任何**线程想要执行 `if` 的 "then" 分支，所有的线程都必须执行——尽管它们中的大多数可能会使用一种称为 " 谓词 "（predication）的技术忽略结果，因为它们一开始就不想进入那个分支。对于 `else` 分支也是如此。

- 如果条件在元素之间往往是一致的（coherent），这种方法效果很好；
- 如果它们或多或少是随机的，效果就不太好。
- **最坏的情况：** 你将总是执行每个 `if` 的两个分支。

循环的工作方式类似——只要至少有一个线程想要继续运行循环，该批次/Warp/Wavefront 中的所有线程都会继续运行。

#### 3. Discard 指令与深度写入

另一个像素着色器特有的指令是 `discard`。像素着色器可以决定 " 杀死 " 当前像素，这意味着它不会被写入。同样，如果批次内的所有像素都被丢弃，着色器单元可以停止并转到另一个批次；但如果至少还有一个线程留存，其余的将被拖着一起走。

DX11 在这里增加了更细粒度的控制，方法是从像素着色器写入输出像素覆盖率（这总是与原始三角形/Z 测试覆盖率进行 " 与 " 运算，以确保着色器不能在图元之外写入，为了其合理性）。这允许着色器丢弃单个样本而不是整个像素；例如，它可以用于在着色器中使用自定义抖动算法实现 Alpha-to-Coverage。

像素着色器还可以写入输出深度（此功能已经存在很长时间了）。根据我的经验，这是**击溃 Early-Z、层级 Z 和 Z 压缩并通常获得尽可能慢路径的绝佳方式**。到现在为止，你已经足够了解这些东西的工作原理，明白为什么会这样了。:)

像素着色器产生多个输出——通常，每个渲染目标（Render Target）对应一个 4 分量向量，目前最多可以有 8 个渲染目标。然后着色器将结果沿管线向下发送到 D3D 称为**" 输出合并器 "（Output Merger）**的地方。这将是我们下次的主题。

#### 4. 无序访问视图（UAVs）

但在我结束之前，还有最后一件事是像素着色器从 D3D11 开始可以做的：它们可以写入**无序访问视图（UAVs）**——这是只有计算着色器和像素着色器可以做的事情。

一般来说，UAV 在计算着色器执行期间取代渲染目标；但与渲染目标不同的是，着色器可以自行确定写入的位置，并且没有隐式的 API 顺序保证（因此名称中有 " 无序访问 "）。目前，我只提到这个功能的存在——当我讲到计算着色器时，我会更多地谈论它。

> **Update:** 在评论中，Steve 提醒了我关于正确的 AMD 术语（这篇文章的第一版没有使用 "Wavefronts" 这个名字，因为我记不起来了），并发布了一个链接，指向 Kayvon Fatahalian 的这个精彩演示，该演示解释了 GPU 上的着色器执行，并附带了更多漂亮的图片，比我愿意制作的要多得多 :)。如果你对着色器核心的工作原理感兴趣，真的应该去看看。

就这样！这次没有一大堆注意事项清单。如果这里遗漏了什么，那是因为我真的忘记了，而不是因为我觉得它太晦涩或太具体而不写在这里。欢迎在评论中指出遗漏之处，我会看看能做些什么。
