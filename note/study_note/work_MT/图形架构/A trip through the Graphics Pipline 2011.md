...
在这一部分，我将处理像素处理的前半部分：调度（dispatch）和实际的像素着色（pixel shading）。实际上，这正是大多数图形程序员在谈论像素处理时脑海中主要想到的内容；至于我们将在下一部分遇到的 Alpha 混合（alpha blend）和后期 Z 测试（late Z）阶段，看起来不过像是事后的一点补充。正如我们将看到的，在硬件层面故事要稍微复杂一些——这也是我将像素处理分成两部分的原因。但我有些操之过急了。当我们进入这个阶段时，要着色的像素（或者确切地说是 2x2 的四像素块，Quads）的坐标以及相关的覆盖掩码（coverage masks），正从光栅化器/Early-Z 单元传送过来——正如我上次指出的，此时三角形的顺序与应用程序提交的顺序完全一致。我们要做的就是获取这一线性的、连续的工作流，将其分发给数百个着色器单元，一旦结果返回，我们需要将其合并回内存更新的一个线性流中。这是一个教科书式的“分叉/合并”（fork/join）并行模式示例。这一部分主要处理“分叉”阶段，即我们扩展宽度的阶段；下一部分将解释“合并”阶段，即我们将数百个流合并回一个流的过程。但在那之前，关于光栅化我还有几句话要说，因为我刚才告诉你的“只有一个四像素块流传入”的说法并不完全正确。

为了给自己辩护一下，我之前告诉你们的情况在很长一段时间里确实是真的，但光栅化是管线中的串行部分，当你把超过 300 个着色器单元投入到一个问题中时，管线的串行部分往往会成为瓶颈。因此 GPU 架构师开始使用多个光栅化器；截至 2010 年，NVidia 使用四个光栅化器，而 AMD 使用两个。顺便提一下，NV 的演示文档中也有一些关于保持 API 顺序要求的说明。特别是，正如我上次提到的，你确实需要在光栅化/Early-Z 之前将图元（primitives）重新排序；如果想在 Alpha 混合之前才做排序（你可能倾向于这么做），是行不通的。光栅化器之间的工作分配是基于我们在 Early-Z 和粗粒度光栅化中已经见过的图块（tiles）进行的。帧缓冲区被划分为图块大小的区域，每个区域分配给其中一个光栅化器。在图元装配（setup）之后，会查询三角形的包围盒以确定将哪些三角形交给哪个光栅化器；大三角形总是会被发送给所有的光栅化器，但小三角形可能只覆盖一个图块，因此只会被发送给拥有该图块的那个光栅化器。这种方案的美妙之处在于它只需要更改工作分配和粗粒度光栅化器（负责遍历图块）；所有只处理单个图块或四像素块的部分（即从层级 Z 往下的管线）都不需要修改。问题在于现在你是根据屏幕位置来划分工作的；这可能会导致光栅化器之间严重的负载不平衡（想象一下几百个微小的三角形都挤在一个图块内的情况），而你对此真的无能为力。但好的一面是，所有给管线增加排序约束的东西（Z 测试/写入顺序、混合顺序）都是依附于特定的帧缓冲区位置的，所以屏幕空间细分是在不破坏 API 顺序的前提下进行的——如果情况并非如此，基于图块的渲染器（tiled renderers）就无法工作了。

现在我们需要更宽的通道！好吧，所以我们得到的不仅仅是一个线性的四像素块坐标加覆盖掩码流，而是两到四个。我们仍然需要将它们分发给数百个着色器单元。是时候再来一个调度单元了！这首先意味着需要另一个缓冲区。但是我们要发送给着色器的批次（batches）有多大呢？这里我再次使用 NVidia 的数据，仅仅因为他们在公开白皮书中提到了这个数字；AMD 可能也在某处声明了该信息，但我不熟悉他们的术语，所以没法直接搜索到。不管怎样，对于 NVidia，分发给着色器单元的单位是 32 个线程，他们称之为“线程束”（Warp）。每个四像素块（Quad）有 4 个像素（每个像素可以作为一个线程处理），所以对于我们要发布的每个着色批次，我们需要从光栅化器抓取 8 个传入的四像素块，然后才能发送一批次给着色器单元（如果有着色器切换或管线刷新，我们可能会发送少于这个数量）。这也是解释为什么我们要处理 2x2 像素的四像素块而不是单个像素的好时机。主要原因是导数（derivatives）。纹理采样器依赖纹理坐标的屏幕空间导数来进行 mip-map 选择和过滤（正如我们在第 4 部分中看到的）；而且，从 Shader Model 3.0 及以后，同样的机制以导数指令的形式直接提供给像素着色器使用。在一个四像素块中，每个像素在同一个块内都有水平和垂直的邻居；这可以用来通过有限差分（finite differencing，归结起来就是几次减法）来估算 x 和 y 方向上参数的导数。这为你提供了一种非常廉价的获取导数的方法，代价是总是必须一次性着色 2x2 的像素组。这在大三角形内部不是问题，但意味着对于三角形边缘生成的四像素块，大约 25-75% 的着色工作是浪费的。这是因为四像素块中的所有像素，即使是被掩盖（masked）的像素，也会被着色。这对于为四像素块中可见的像素产生正确的导数是必要的。那些不可见但仍被着色的像素被称为“辅助像素”（helper pixels）。

这里有一个小三角形的插图：三角形与 4 个四像素块相交，但只在其中的 3 个块中生成可见像素。此外，在这 3 个四像素块的每一个中，实际上只有一个像素被覆盖（每个像素区域的采样点描绘为黑色圆圈）——被填充的像素描绘为红色。每个部分覆盖的四像素块中的其余像素是辅助像素，用较浅的颜色绘制。这个插图应该清楚地表明，对于小三角形，被着色的像素总数中有很大一部分是辅助像素，这吸引了一些关于如何合并相邻三角形的四像素块的研究关注。然而，虽然这些优化很聪明，但目前的 API 规则不允许这样做，目前的硬件也不做这些。当然，如果硬件供应商在某个时候认为四像素块上的浪费着色工作是一个严重到必须解决的问题，这可能会改变。

像素着色器的另一个独特功能是属性插值（attribute interpolation）——所有其他着色器类型，无论是我们目前见过的（VS）还是我们要讨论的（GS, HS, DS, CS），都直接从前一个着色器阶段或内存中获取输入，但像素着色器在它们前面有一个额外的插值步骤。我在讨论 Z 时已经稍微谈到了这一点，那是我们看到的第一个插值属性。其他插值属性的工作方式大致相同；在三角形装配期间为它们计算一个平面方程（GPU 可能会选择稍微推迟这个计算，例如直到已知三角形至少有一个图块通过了层级 Z 测试，但这不影响我们这里的讨论），然后在像素着色期间，有一个单独的单元使用四像素块的像素位置和我们刚刚计算的平面方程执行属性插值。更新：Marco Salvi 在评论中指出，虽然过去有专用的插值器，但现在的趋势是只让它们返回重心坐标（barycentric coordinates）以代入平面方程。实际的求值（每个属性两个乘加运算）可以在着色器单元中完成。所有这些都不应该令人惊讶，但还有一些额外的插值类型需要讨论。首先是“常量”（constant）插值器，正如其名，它们在整个图元上是恒定的，并从“主导顶点”（leading vertex，在图元装配期间确定是哪个顶点）获取每个顶点属性的值。硬件可能有针对此的快速路径，或者只是设置一个相应的平面方程；无论哪种方式都可以。然后是非透视（no-perspective）插值。这通常会以不同的方式设置平面方程；透视校正插值的平面方程要么是通过将每个顶点的属性值除以相应的 w 来进行基于 X、Y 的插值，要么是通过构建三角形边缘向量来进行重心插值。然而，当非透视插值属性的平面方程被设置为基于 X、Y 的插值且不将每个顶点的值除以相应的 w 时，计算成本最低。

接下来，我们有“质心”（centroid）插值。这是一个标志，不是一个单独的模式；它可以与透视和非透视模式结合使用（但不能与常量插值结合，因为那没有意义）。它的命名也很糟糕，而且除非启用了多重采样（multisampling），否则它就是一个空操作。在开启多重采样的情况下，它是解决一个实际问题的稍微有点“黑客”性质的方案。问题在于，使用多重采样时，我们在光栅化器中评估多个采样点的三角形覆盖范围，但我们要对每个像素只进行一次实际着色。诸如纹理坐标之类的属性将在像素中心位置进行插值，就好像整个像素都被图元覆盖一样。这会导致问题，例如：我们有一个被图元部分覆盖的像素；四个小圆圈描绘了 4 个采样点（这是默认的 4x MSAA 模式），而中间的大圆圈描绘了像素中心。注意大圆圈在图元之外，任何对它的“插值”值实际上都是线性外推（extrapolation）；如果应用程序使用纹理图集（texture atlases），这是一个问题。根据三角形的大小，像素中心的值可能会偏离很远。质心采样解决了这个问题。最初的解释是 GPU 取图元覆盖的所有样本，计算它们的质心，并在该位置采样（因此得名）。通常后面会补充说这只是一个概念模型，GPU 可以自由地以不同方式进行，只要它们选择的采样点在图元内即可。如果你觉得硬件真的会去数覆盖的采样点，把它们加起来，然后除以总数，那欢迎加入我们的阵营（意指大家一开始都容易这么想，但事实并非如此）。实际发生的情况是：如果所有采样点都覆盖图元，插值照常进行，即在像素中心（对于所有合理的采样模式，它恰好是所有采样位置的质心）。如果并非所有采样点都覆盖三角形，硬件会选择其中一个覆盖图元的采样点并在那里进行评估。所有覆盖的采样点（根据定义）都在图元内，所以这行得通。这种选择过去是任意的（即留给硬件决定）；我相信现在 DX11 实际上规定了它是如何完成的，但这更多是为了在不同硬件之间获得一致的结果，而不是 API 用户实际上会关心的东西。如前所述，这有点“黑客”手段。它也倾向于搞乱具有部分覆盖像素的四像素块的导数计算——算你倒霉。我能说什么呢，这可能是工业强度的胶带，但它仍然是胶带。最后（DX11 新增功能！）还有“拉取模型”（pull-model）属性插值。常规属性插值在像素着色器启动之前自动完成；拉取模型插值将实际执行插值的指令添加到像素着色器中。这允许着色器计算自己的位置来采样值，或者只在某些分支中插值属性而在其他分支中不插值。归根结底，这是让像素着色器能够在运行时向插值单元发送额外请求。

关于实际的着色器主体，一般的着色器原理在 API 文档中已经解释得很清楚了，所以我不会谈论单个指令是如何工作的；通常，答案是“如你所期望的那样”。然而，有一些关于像素着色器执行的有趣之处值得探讨。第一个是：纹理采样！等等，我不是在第 4 部分已经长篇大论过纹理采样器了吗？是的，但那是纹理采样器方面的事情——如果你还记得的话，有一点是关于纹理缓存未命中非常频繁，以至于采样器通常被设计为每个请求（也就是 16-32 个像素，记得吗！）能承受至少一次主内存未命中而不发生停顿。那是很多个时钟周期——数以百计。让完美的 ALU（算术逻辑单元）在这一切发生时闲置将是巨大的浪费。所以着色器单元实际做的是在发出纹理采样后切换到另一个批次；然后当那个批次发出纹理采样（或完成）时，它切换回之前的批次之一，并检查纹理样本是否已经到达。只要每个着色器单元在任何给定时间都有几个批次可以处理，这就能很好地利用可用资源。不过，这确实增加了单个批次完成的延迟——再次强调，这是延迟与吞吐量的权衡。到现在你应该知道 GPU 上哪一方获胜了：吞吐量！永远是吞吐量。这里要注意的一点是，同时保持多个批次（或 NVidia 硬件上的“Warps”，或 AMD 的“Wavefronts”）运行需要更多的寄存器。如果一个着色器需要大量寄存器，着色器单元能保留的 Warp 就会变少；如果它们变少了，那么在某个时刻你会耗尽那些不需要等待纹理结果的可运行批次，这种几率就会变高。如果没有可运行的批次，你就倒霉了，必须停顿直到其中一个批次取回结果。这很不幸，但这类事情的硬件资源是有限的——如果内存用完了，那就是用完了，没得商量。

我还没谈到的另一点：着色器中的动态分支（即循环和条件语句）。在着色器单元中，每个批次的所有元素的工作通常是步调一致（lockstep）进行的。所有“线程”在同一时间运行相同的代码。这意味着 `if` 语句有点棘手：如果任何线程想要执行 `if` 的“then”分支，所有的线程都必须执行——尽管它们中的大多数可能会使用一种称为“谓词”（predication）的技术忽略结果，因为它们一开始就不想进入那个分支。对于 `else` 分支也是如此。如果条件在元素之间往往是一致的（coherent），这种方法效果很好；如果它们或多或少是随机的，效果就不太好。最坏的情况是，你将总是执行每个 `if` 的两个分支。哎哟。循环的工作方式类似——只要至少有一个线程想要继续运行循环，该批次/Warp/Wavefront 中的所有线程都会继续运行。另一个像素着色器特有的指令是 `discard`。像素着色器可以决定“杀死”当前像素，这意味着它不会被写入。同样，如果批次内的所有像素都被丢弃，着色器单元可以停止并转到另一个批次；但如果至少还有一个线程留存，其余的将被拖着一起走。DX11 在这里增加了更细粒度的控制，方法是从像素着色器写入输出像素覆盖率（这总是与原始三角形/Z 测试覆盖率进行“与”运算，以确保着色器不能在图元之外写入，为了其合理性）。这允许着色器丢弃单个样本而不是整个像素；例如，它可以用于在着色器中使用自定义抖动算法实现 Alpha-to-Coverage。像素着色器还可以写入输出深度（此功能已经存在很长时间了）。根据我的经验，这是击溃 Early-Z、层级 Z 和 Z 压缩并通常获得尽可能慢路径的绝佳方式。到现在为止，你已经足够了解这些东西的工作原理，明白为什么会这样了。:) 像素着色器产生多个输出——通常，每个渲染目标（Render Target）对应一个 4 分量向量，目前最多可以有 8 个渲染目标。然后着色器将结果沿管线向下发送到 D3D 称为“输出合并器”（Output Merger）的地方。这将是我们下次的主题。

但在我结束之前，还有最后一件事是像素着色器从 D3D11 开始可以做的：它们可以写入无序访问视图（UAVs）——这是只有计算着色器和像素着色器可以做的事情。一般来说，UAV 在计算着色器执行期间取代渲染目标；但与渲染目标不同的是，着色器可以自行确定写入的位置，并且没有隐式的 API 顺序保证（因此名称中有“无序访问”）。目前，我只提到这个功能的存在——当我讲到计算着色器时，我会更多地谈论它。更新：在评论中，Steve 提醒了我关于正确的 AMD 术语（这篇文章的第一版没有使用“Wavefronts”这个名字，因为我记不起来了），并发布了一个链接，指向 Kayvon Fatahalian 的这个精彩演示，该演示解释了 GPU 上的着色器执行，并附带了更多漂亮的图片，比我愿意制作的要多得多 :)。如果你对着色器核心的工作原理感兴趣，真的应该去看看。就这样！这次没有一大堆注意事项清单。如果这里遗漏了什么，那是因为我真的忘记了，而不是因为我觉得它太晦涩或太具体而不写在这里。欢迎在评论中指出遗漏之处，我会看看能做些什么。
