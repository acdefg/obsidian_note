...
modified: 星期日, 六月 28日 2026, 5:37:03 下午
---
...
第二点：
先 getchar() 挂起进程，用 ps axf 命令看到父子进程调度关系：
...
# 作业要求
实验内容  
1. 编写 fork()，vfork() 创建多进程程序，调用 exec()，wait()  
函数，通过输出结果观察进程的执行过程，理解函数在  
创建进程中的作用。  
2. 编写 fork() 多进程程序，进程间模拟信号通信，包括信号  
发送、捕捉与处理。  
3. 编写 fork() 多进程程序，进程间实现有名管道和无名管道  
通信。具体要求如下：  
⚫ 编写 1 个服务器程序 server.c 和 2 个客户端程序 client1.c，client2.c，  
