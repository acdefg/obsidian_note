...
实际上 PBD 还有一个巨大的优势在于算法仅仅需要访问顶点信息，而其他算法（比若说隐式积分都需要做很多次内存访问）
...
## 积分方法
求解方法主要分为 Euler Method 和 Verlet Integration
...
矩阵求解
1. 对以上隐式积分的线性化后的方程利用 rram 模拟特性求解
	需要阵列大小和 cloth 的大小一致
2. 使用牛顿雅可比等方法循环迭代，使用硬件加速
矩阵加减
3. 在算法中求解力的部分，需要在过程中加上 ：对 v[128 * 128] x[128 * 128] 进行更新
	1. 重力
	2. 弹力
	3. 阻力
	4. 阻尼
