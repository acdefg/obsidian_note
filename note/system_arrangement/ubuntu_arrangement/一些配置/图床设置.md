...
# 推荐链接
图床介绍，以及好几个图床上传方案和选择工具：https://blog.csdn.net/qq_38768365/article/details/107091900/ - PicGo 的安装与使用

> 本篇采用 Picgo 结合 gitee、github、ssms
...
```
1.3.1 需要手动到设置中打开使用内置剪切板上传，才能使用之前安装的 xclip

## obsidian 插件
下载 Image auto upload Plugin，安装完成后，将默认上传器设置为 PicGo 即可
# 图床存储
## 使用中
阿里云 oss
40GB/9rmb
### ssms
https://sm.ms - ssms 官网
token 地址：https://sm.ms/home/
	注册完成后，通过 user-> dashboard ->Api token
- 特点：
	1. 单次上传不超过 5MB
	2. 每次不超文件过诗歌
...
### gitee
使用 gitee 需要安装插件，该插件需要 [[安装node.js]]
- 记录：windows 中使用

### github
https://www.jianshu.com/p/d51258ef5484 - github token 设置参考
以下 github 内容参考上文：
- github 配置 token：
	1. 新建仓库放图片
	2. 点击右上角头像 ->Settings->Developer settings(botton)->Personal access tokens，点击 Generate new token
...
- 在 Picgo 里配置：
	- 仓库名：用户名/仓库名
	- 分支：一般为 master，也可以设置其他，只要是你存东西的那个就行
...
- 记录：
	...
	- ubuntu 中用于 obsidian 图片上传

> 据说存在上传量过大被官方发现封号的情况

### 阿里云
实名认证注册账号
开通 oss 服务，购买 40GB/年，本地冗余存储方案即可，作为图床/笔记备份够了
之后开通 bucket 和新建 accesskey，具体参考这篇 [手把手教你搭建阿里云图床(PicGo+Typora+阿里云OSS)，新手小白一看就会-阿里云开发者社区](https://developer.aliyun.com/article/1465963)
- 图床：使用 picgo 配置阿里云相关信息，用本地 excel 记录下来所有的 accesskeyID accesskey secret
- obsidian 笔记备份： 下载插件 `remotely save`
- ![image.png |300|275x244](https://imag060625.oss-cn-beijing.aliyuncs.com/img/20251109180935786.png) 其中 region 是 endpoint 的第一个字段

## 未使用
### 腾讯云 COS
- 好像有 50G 免费空间，超出计费，据说只限于前几个月免费
